...
export class SecretController {
    public static readonly getSecretHistory = async (c: Context) => {
        try {
            ...
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to view secret history in Production." }, 403);
            }
    ...
    public static readonly getSecretDiff = async (c: Context) => {
        try {
            ...
            const env_type = await EnvTypeService.getEnvType(env_type_id);
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to view secret diffs in Production." }, 403);
            }
    ...
    public static readonly getSecretVariableTimeline = async (c: Context) => {
        try {
            ...
            const env_type = await EnvTypeService.getEnvType(env_type_id);
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to view secret timeline in Production." }, 403);
            }
    ...
    public static readonly rollbackSecretsToPitId = async (c: Context) => {
        try {
            ...
            const env_type = await EnvTypeService.getEnvType(env_type_id);
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to rollback secrets in Production." }, 403);
            }
            ...
            if (rollbackOperations.length > 0) {
                await SecretStorePiTService.createSecretStorePiT({
                    ...
                    app_id,
                    env_type_id,
                    change_request_message: rollback_message || `Rollback to PiT ${pit_id}: ${rollbackOperations.length} secrets affected`,
                    user_id,
        ...
        try {
            ...
            if (rollbackOperations.length > 0) {
                await SecretStorePiTService.createSecretStorePiT({
                    ...
                    app_id,
                    env_type_id,
                    change_request_message: rollback_message || `Rollback to ${targetTimestamp.toISOString()}: ${rollbackOperations.length} secrets affected`,
                    user_id,
                    envs: rollbackOperations
                });
    ...
    public static readonly rollbackSecretVariableToPitId = async (c: Context) => {
        try {
            ...
            await SecretStorePiTService.createSecretStorePiT({
                ...
                app_id,
                env_type_id,
                change_request_message: rollback_message || `Rollback secret ${key} to PiT ${pit_id}: ${rollbackOperation.previous_value} → ${rollbackOperation.target_value}`,
                user_id,
                envs: [{
    ...
    public static readonly rollbackSecretVariableToTimestamp = async (c: Context) => {
        try {
            ...
            await SecretStorePiTService.createSecretStorePiT({
                ...
                app_id,
                env_type_id,
                change_request_message: rollback_message || `Rollback secret ${key} to ${targetTimestamp.toISOString()}: ${rollbackOperation.previous_value} → ${rollbackOperation.target_value}`,
                user_id,
                envs: [{
                    key: rollbackOperation.key,
                    value: rollbackOperation.value,
    ...
    public static readonly createSecretWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            ...
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to create secrets in Production." }, 403);
            }
    ...
    public static readonly updateSecretWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            const { key } = c.req.param();
            ...
            // env_type's name is "Production", user must have admin permissions
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to update secrets in Production." }, 403);
            }
    ...
    public static readonly deleteSecretWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            ...
            // env_type's name is "Production", user must have admin permissions
            if (env_type.is_protected && (!permissions.is_admin || !permissions.is_master)) {
                return c.json({ error: "You do not have permission to delete secrets in Production." }, 403);
            }
    ...
    public static readonly batchCreateSecretsWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            ...
            const existingKeys = existingSecrets
            ...
            await SecretStorePiTService.createSecretStorePiT({
                ...
                app_id,
                env_type_id,
                change_request_message: change_message || `Batch created ${modEnvs.length} secrets: ${modEnvs.map(env => env.key).join(", ")}`,
                user_id,
    ...
    public static readonly batchUpdateSecretsWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            ...
            if (nonExistingKeys.length > 0) {
                return c.json({ error: `Secrets do not exist for keys: ${nonExistingKeys.join(", ")}` }, 400);
            }
            ...
            await SecretStorePiTService.createSecretStorePiT({
                ...
                app_id,
                env_type_id,
                change_request_message: change_message || `Batch updated ${modEnvs.length} secrets: ${modEnvs.map(env => env.key).join(", ")}`,
                user_id,
                envs: modEnvs.map(env => ({
                    key: env.key,
                    value: env.value,
    ...
    public static readonly batchDeleteSecretsWithPiT = async (c: Context) => {
        try {
            const org_id = c.get("org_id");
            const user_id = c.get("user_id");
            ...
            await SecretStorePiTService.createSecretStorePiT({
                ...
                app_id,
                env_type_id,
                change_request_message: change_message || `Batch deleted ${secretsToDelete.length} secrets: ${keys.join(", ")}`,
                user_id,
