...
export async function up(db: Kysely<Database>): Promise<void> {
    ...
    await db.schema
        ...
        .addColumn("created_at", "timestamp", (col) => col.defaultTo('now()'))
        .addColumn("updated_at", "timestamp", (col) => col.defaultTo('now()'))
        .addForeignKeyConstraint("fk_env_store_pit_change_request_env_store_pit", ["env_store_pit_id"], "env_store_pit", ["id"], cb =>
            cb.onDelete("cascade")
        )
        .execute();
    ...
    await db.schema
        ...
        .addForeignKeyConstraint("fk_secret_store_pit_env_type", ["env_type_id"], "env_type", ["id"], cb =>
            cb.onDelete("cascade")
        )
    ...
    await db.schema
        ...
        .addColumn("created_at", "timestamp", (col) => col.defaultTo('now()'))
        .addColumn("updated_at", "timestamp", (col) => col.defaultTo('now()'))
        .addForeignKeyConstraint("fk_secret_store_pit_change_request_secret_store_pit", ["secret_store_pit_id"], "secret_store_pit", ["id"], cb =>
            cb.onDelete("cascade")
        )
        .execute();
}
...
export async function down(db: Kysely<Database>): Promise<void> {
    await db.schema
        .alterTable("app")
        .dropColumn("enable_secrets")
        .execute();

    await db.schema
        .alterTable("app")
        .dropColumn("is_managed_secret")
        .execute();
