...
export class InviteService {
  public static createOrgInvite = async (
    email: string,
  ) => {
    ...
    public static createUserInvite = async (
        email: string,
        org_id: string,
        role_id: string,
    ) => {
    ...
    public static getOrgInviteByCode = async (
        invite_code: string,
    ) => {
        const db = await DB.getInstance();
    ...
    public static getUserInviteByCode = async (
        invite_code: string,
    ) => {
        const db = await DB.getInstance();
        const invite = await db
    ...
    public static deleteInvite = async (
        invite_id: string,
    ) => {
        const db = await DB.getInstance();
        await db
            .deleteFrom('invite_org')
            .where('id', '=', invite_id)
            .executeTakeFirstOrThrow();
    }

    public static deleteUserInvite = async (
        invite_id: string,
    ) => {
        const db = await DB.getInstance();
        await db
            .deleteFrom('invite_user')
            .where('id', '=', invite_id)
            .executeTakeFirstOrThrow();
    }

    public static getAllUserInvites = async (
        org_id: string,
    ) => {
        const db = await DB.getInstance();
