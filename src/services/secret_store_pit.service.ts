...
export class SecretStorePiTService {
    ...
    public static getSecretStorePiTById = async ({
        id,
    }: {
        id: string;
    }) => {
    ...
    public static getSecretStorePiTsByVariable = async ({
        ...
        const pits = await db
            ...
            .where("secret_store_pit_change_request.key", "=", key)
    ...
    public static getEnvDiff = async ({
        ...
        const [fromState, toState] = await Promise.all([
            this.getEnvsTillPiTId({ org_id, app_id, env_type_id, secret_store_pit_id: from_pit_id }),
            this.getEnvsTillPiTId({ org_id, app_id, env_type_id, secret_store_pit_id: to_pit_id })
        ]);
